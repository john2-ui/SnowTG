...
static void validate_indexes(const struct nsock *sk) {
  ...
  while (list_node != NULL) {
    ...
    CHECK(list_node->prev ==
          (list_count == 0
               ? NULL
               : rb_entry(rb_prev(tree_node), struct tcp_ofo_seg, rb)));
  ...
  CHECK(sk->u.tcp.ofo_tail ==
        (sk->u.tcp.ofo == NULL
             ? NULL
             : rb_entry(rb_last(&sk->u.tcp.ofo_tree), struct tcp_ofo_seg, rb)));
}
...
static void test_overlap_trimming_and_fin(void) {
  ...
  init_socket(&sk, 0, 64);
  CHECK(tcp_test_ofo_insert(&sk, 20, (const uint8_t *)original, 10, 0) == 0);
  CHECK(tcp_test_ofo_insert(&sk, 15, (const uint8_t *)overlap, 20, 0) == 0);
  validate_indexes(&sk);
...
static void test_receive_window_and_capacity(void) {
  ...
  CHECK(tcp_test_ofo_insert(&sk, 90, (const uint8_t *)payload, 20, 0) == 0);
  CHECK(tcp_test_ofo_insert(&sk, 110, (const uint8_t *)payload, 16, 0) == 0);
  CHECK(tcp_test_ofo_insert(&sk, 122, (const uint8_t *)payload, 1, 0) == 0);
  validate_indexes(&sk);
  ...
  for (i = 0; i < TCP_OFO_MAX_SEGS; i++) {
    CHECK(tcp_test_ofo_insert(&sk, i * 2, (const uint8_t *)payload, 1, 0) == 0);
  }
  CHECK(tcp_test_ofo_insert(&sk, TCP_OFO_MAX_SEGS * 2, (const uint8_t *)payload,
                            1, 0) == -1);
  validate_indexes(&sk);
  tcp_test_ofo_purge(&sk);
}
...
static void test_sequence_wraparound(void) {
  ...
  CHECK(tcp_test_ofo_insert(&sk, UINT_MAX - 7, (const uint8_t *)a, 1, 0) == 0);
