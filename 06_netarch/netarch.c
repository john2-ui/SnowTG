...
int main(int argc, char *argv[]) {
         ...
         * once races (a half-initialized ring can be observed), which crashes as
         * soon as the bridged NIC delivers its first broadcast frame.
         */
        struct inout_ring *ring = ring_instance();
        arp_table_instance();
        ...
        while (1) {
                ...
                if (nb_rx > 0)
                        rte_ring_sp_enqueue_burst(ring->in, (void **)rx, nb_rx,
                                                  NULL);
