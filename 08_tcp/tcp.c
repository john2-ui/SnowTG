/**
 * @file tcp.c
 * @brief TCP TCB management, passive 3-way handshake, and packet encode/egress.
 *
 * Inbound path (worker):  tcp_handle -> state handler -> send_buf fragment
 * Outbound path (worker): tcp_out     -> arp resolve   -> out ring -> NIC
 */
#include "tcp.h"
...
static struct tcp_table *t_instance = NULL;

/** @brief Convert a TCP status enum to a short log label. */
...
/**
 * @brief Format common TCP flag bits into a short string for logging.
 * @warning Uses a static buffer; not safe for nested calls in one expression.
 */
...
struct tcp_stream *tcp_stream_create(uint32_t src_ip, uint32_t dst_ip,
        ...
        /*
         * Server-only for now: every new TCB starts in LISTEN. A real stack
         * would distinguish active vs passive open here.
         */
        ...
        /* Randomize the initial sequence number (ISN). */
...
int tcp_stream_handle_listen(struct tcp_stream *stream,
        ...
        /* Handshake step 1: peer SYN -> queue SYN+ACK, enter SYN_RECV. */
        ...
        /* Reply swaps the ports from the inbound segment. */
        ...
        /* data_off: upper 4 bits = header length in 32-bit words (5 => 20 B).
         */
        fragment->data_off = (5 << 4);
        fragment->rx_win = TCP_INITIAL_WINDOW_SIZE; /* host order */
        fragment->tcp_urp = 0;
        fragment->opt_len = 0;
        ...
        LOG_INFO("tcp handshake [1/3] SYN rx " IP_FMT ":%u -> " IP_FMT
                 ...
                 IP_ARG(stream->src_ip), rte_be_to_cpu_16(stream->src_port),
                 IP_ARG(stream->dst_ip), rte_be_to_cpu_16(stream->dst_port),
...
int tcp_stream_handle_syn_recv(struct tcp_stream *stream,
        ...
        /* Handshake step 3: peer ACK of our ISN+1 -> ESTABLISHED. */
        ...
        if (acknum == expect_ack) {
                LOG_INFO(
                    "tcp handshake [3/3] ACK rx " IP_FMT ":%u -> " IP_FMT
                    ":%u seq=%u ack=%u (expect=%u)",
                    IP_ARG(stream->src_ip), rte_be_to_cpu_16(stream->src_port),
                    IP_ARG(stream->dst_ip), rte_be_to_cpu_16(stream->dst_port),
                    ntohl(tcp_hdr->sent_seq), acknum, expect_ack);
        } else {
                LOG_WARN(
                    "tcp handshake ACK mismatch " IP_FMT ":%u -> " IP_FMT
                    ":%u ack=%u expect=%u",
                    IP_ARG(stream->src_ip), rte_be_to_cpu_16(stream->src_port),
                    IP_ARG(stream->dst_ip), rte_be_to_cpu_16(stream->dst_port),
                    acknum, expect_ack);
        ...
        /* TODO: Only transition when acknum matches expect_ack. */
        stream->status = TCP_STATUS_ESTABLISHED;
        LOG_INFO("tcp handshake done, status %s -> %s " IP_FMT ":%u <-> " IP_FMT
                 ":%u",
...
/** @brief Locate the IPv4 header that follows the Ethernet header. */
...
int tcp_handle(struct rte_mbuf *mbuf) {
        ...
        /* Learn peer MAC so the SYN+ACK path need not wait on ARP. */
        ...
        case TCP_STATUS_SYN_RECV:
                ...
                break;
        case TCP_STATUS_SYN_SENT:
                /* TODO: client-side handshake completion. */
                break;
        case TCP_STATUS_ESTABLISHED: {
                /* TODO: deliver payload to the socket receive ring. */
...
/**
 * @brief Encode Ethernet/IPv4/TCP (+ optional payload) into @p msg.
 *
 * @c fragment->sent_seq / @c recv_ack / @c rx_win are host order; ports are
...
/**
 * @brief Allocate an mbuf and fill it from @p fragment.
 * @return New mbuf, or NULL on allocation failure.
 */
...
void tcp_out(struct rte_mempool *mp) {
        ...
        for (stream = table->tcb_set; stream != NULL; stream = stream->next) {
                /* SYN_RECV carries SYN+ACK; ESTABLISHED carries data. */
                if (stream->status != TCP_STATUS_SYN_RECV &&
                    stream->status != TCP_STATUS_ESTABLISHED)
                        continue;

                struct tcp_fragment *fragment = NULL;
                if (rte_ring_sc_dequeue(stream->send_buf, (void **)&fragment) <
                    0)
                        continue;

                /* stream->src_* is the remote peer; replies go there. */
                uint32_t peer_ip = stream->src_ip;
                uint8_t *dst_mac = arp_lookup(peer_ip);
                if (dst_mac == NULL) {
                        /* Peer MAC unknown: ARP request, then retry later. */
                ...
                struct rte_mbuf *tcp_buf =
                    tcp_build_pkt(mp, g_net.local_ip, peer_ip, g_net.local_mac,
                                  dst_mac, fragment);
                if (tcp_buf == NULL) {
                        rte_free(fragment);
                        continue;
                ...
                if ((fragment->tcp_flags &
                    ...
                    (RTE_TCP_SYN_FLAG | RTE_TCP_ACK_FLAG)) {
                        /* Handshake step 2: local SYN+ACK on the wire. */
